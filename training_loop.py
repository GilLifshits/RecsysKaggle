...
#
#         # Combine positive and negative similarities into logits
#         logits = torch.cat([scaled_similarity.unsqueeze(1), negative_scaled_similarity.unsqueeze(1)], dim=0)
#
#         # Generate labels: index 0 corresponds to positive samples
#         targets = torch.cat([
#             torch.zeros(len(scaled_similarity), dtype=torch.long, device=device),
#             torch.ones(len(negative_scaled_similarity), dtype=torch.long, device=device)
#         ])
#
#         # Compute InfoNCE loss
#         loss = F.cross_entropy(logits, targets)
